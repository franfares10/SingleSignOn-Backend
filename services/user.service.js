...
const createUser = async function (user) {
  ...
  try {
    ...
    var isUserRegisteredInTenant = await checkEmailTenant(
      user.email,
      user.tenant
    );

    if (!isUserRegistered) {
      //if not registered --> we must create the user in both Collections (Credentials && Specified Tenant)
      ...
      if (!isUserRegisteredInTenant) {
        //if registered in credentials && not registered in the specified tenant --> we must create the user in the Tenant Collection

        var savedUserInTenant = await registerUserInTenant(tenantUser);
        console.log("04- El usuario se guardó en el tenant correspondiente");
        return savedUserInTenant;
      } else {
        //The user is registered in both Tenant and Credentials Collection
...
const deleteUser = async function (user) {
  try {
    ...
    if (isUserRegistered && isUserRegisteredInTenant) {
      var response = await Credentials.deleteOne({
        email: user.email,
        tenant: user.tenant,
      });
      if (user.tenant === "cms") {
